...
<body class="bg-[#f8f9fa] text-slate-800 antialiased overflow-x-hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-0 shadow-2xl border border-light overflow-hidden">
                        <div class="bg-white p-10 border-r border-light relative group">
                            ...
                            <div class="flex items-end justify-between">
                                <span class="text-5xl font-black text-navy leading-none">{{ $staff->count() }}</span>
                        ...
                        <div class="bg-primary p-10 relative group text-white border-r border-white/5">
                            ...
                            <div class="flex items-end justify-between">
                                <span class="text-5xl font-black text-white leading-none">{{ $academics->count() }}</span>
                        ...
                        <div class="bg-white p-10 border-r border-light relative group">
                            ...
                            <div class="flex items-end justify-between">
                                <span class="text-5xl font-black text-navy leading-none">{{ $news->count() }}</span>
                        ...
                        <div class="bg-navy p-10 relative group text-white">
                            ...
                            <div class="flex items-end justify-between">
                                <span class="text-5xl font-black text-white leading-none">{{ $announcements->count() }}</span>
                ...
                <div x-show="section === 'berita'" x-cloak x-transition.opacity>
                    <div class="bg-white shadow-2xl overflow-hidden border border-light">
                        <div class="p-10 border-b border-light flex justify-between items-center bg-white sticky top-0 z-10">
                            ...
                            <button @click="openAddModal('news', '{{ route('admin.news.store') }}')" class="btn-primary">+ Tambah Berita</button>
                        </div>
                        <table class="w-full text-left">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b border-light hover:bg-light/50 transition-colors">
                                    <td class="px-10 py-8">
                                        <div class="font-black text-navy uppercase text-sm">{{ $item->title }}</div>
                                        <div class="text-[10px] font-bold text-slate-400 mt-2 tracking-widest">SLUG: {{ $item->slug }}</div>
                                    </td>
                                    <td class="px-10 py-8 uppercase text-[10px] font-black text-primary">{{ $item->category }}</td>
                                    <td class="px-10 py-8"><span class="px-4 py-1 bg-green-100 text-green-600 text-[9px] font-black uppercase rounded-full">{{ $item->is_published ? 'Published' : 'Draft' }}</span></td>
                                    <td class="px-10 py-8">
                                        <div class="flex gap-2">
                                            <button @click="openEditModal('news', '{{ route('admin.news.update', $item->id) }}', {{ $item }})" class="w-8 h-8 bg-blue-500 text-white flex items-center justify-center hover:bg-navy transition-all rounded"><i class="fa fa-edit text-xs"></i></button>
                ...
                <div x-show="section === 'pengumuman'" x-cloak x-transition.opacity>
                    <div class="bg-white shadow-2xl overflow-hidden border border-light">
                        <div class="p-10 border-b border-light flex justify-between items-center bg-white sticky top-0 z-10">
                            ...
                            <button @click="openAddModal('announcement', '{{ route('admin.announcements.store') }}')" class="btn-secondary">+ Buat Pengumuman</button>
                        </div>
                        <table class="w-full text-left">
                            <thead class="bg-light/50">
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr class="border-b border-light hover:bg-light/50 transition-colors">
                                    <td class="px-10 py-8">
                                        <div class="font-black text-navy uppercase text-sm">{{ $announcement->title }}</div>
                                        <p class="text-[10px] font-bold text-slate-400 mt-2 italic capitalize">{{ Str::limit($announcement->content, 80) }}</p>
                                    </td>
                                    <td class="px-10 py-8 uppercase text-[10px] font-black {{ $announcement->priority === 'Urgent' ? 'text-red-500' : 'text-primary' }}">
                                        {{ $announcement->priority }} 
                                        @if($announcement->priority === 'Urgent')<i class="fa fa-exclamation-triangle ml-1"></i>@endif
                                    </td>
                                    <td class="px-10 py-8"><span class="px-4 py-1 {{ $announcement->is_active ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600' }} text-[9px] font-black uppercase rounded-full">{{ $announcement->is_active ? 'Aktif' : 'Nonaktif' }}</span></td>
                                    <td class="px-10 py-8">
                                        <div class="flex gap-2">
                                            <button @click="openEditModal('announcement', '{{ route('admin.announcements.update', $announcement->id) }}', {{ $announcement }})" class="w-8 h-8 bg-blue-500 text-white flex items-center justify-center hover:bg-navy transition-all rounded"><i class="fa fa-edit text-xs"></i></button>
                ...
                <div x-show="section === 'fasilitas'" x-cloak x-transition.opacity>
                    <div class="bg-white shadow-2xl overflow-hidden border border-light">
                        <div class="p-10 border-b border-light flex justify-between items-center bg-white sticky top-0 z-10">
                            ...
                            <button @click="openAddModal('academic', '{{ route('admin.academics.store') }}', {type: 'fasilitas'})" class="btn-primary">+ Tambah Fasilitas</button>
                        </div>
                        <table class="w-full text-left">
                            <thead class="bg-light/50">
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr class="border-b border-light hover:bg-light/50 transition-colors">
                                    ...
                                    <td class="px-10 py-8 text-sm text-slate-600">{{ Str::limit($item->description, 50) }}</td>
                                    <td class="px-10 py-8">
                                        @if($item->image)
                                        <img src="{{ asset('storage/' . $item->image) }}" class="w-12 h-12 object-cover rounded-lg border border-slate-200" alt="{{ $item->title }}">
                                        @else
                ...
                <div x-show="section === 'ekskul'" x-cloak x-transition.opacity>
                    <div class="bg-white shadow-2xl overflow-hidden border border-light">
                        <div class="p-10 border-b border-light flex justify-between items-center bg-white sticky top-0 z-10">
                            ...
                            <button @click="openAddModal('academic', '{{ route('admin.academics.store') }}', {type: 'ekskul'})" class="btn-primary">+ Tambah Ekskul</button>
                        </div>
                        <table class="w-full text-left">
                            <thead class="bg-light/50">
                                <tr>
                ...
                <div x-show="section === 'kurikulum'" x-cloak x-transition.opacity>
                    <div class="bg-white shadow-2xl overflow-hidden border border-light">
                        <div class="p-10 border-b border-light flex justify-between items-center bg-white sticky top-0 z-10">
                            ...
                            <button @click="openAddModal('academic', '{{ route('admin.academics.store') }}', {type: 'kurikulum'})" class="btn-primary">+ Tambah Data</button>
                        </div>
                        <table class="w-full text-left">
                            <thead class="bg-light/50">
                                <tr>
                            ...
                            <tbody>
                                @forelse($kurikulum as $academic)
                                <tr class="border-b border-light hover:bg-light/50 transition-colors">
                                    <td class="px-10 py-8">
                                        <div class="font-black text-navy uppercase text-sm">{{ $academic->title }}</div>
                                    </td>
                                    <td class="px-10 py-8 uppercase text-[10px] font-black text-primary">{{ $academic->type }}</td>
                                    <td class="px-10 py-8 text-sm text-slate-600">{{ Str::limit($academic->description, 50) }}</td>
                                    <td class="px-10 py-8">
                                        <div class="flex gap-2">
                                            <button @click="openEditModal('academic', '{{ route('admin.academics.update', $academic->id) }}', {{ $academic }})" class="w-8 h-8 bg-blue-500 text-white flex items-center justify-center hover:bg-navy transition-all rounded"><i class="fa fa-edit text-xs"></i></button>
                                            <form method="POST" action="{{ route('admin.academics.destroy', $academic->id) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Yakin hapus data ini?')" class="w-8 h-8 bg-secondary text-white flex items-center justify-center hover:bg-navy transition-all rounded"><i class="fa fa-trash text-xs"></i></button>
                                            </form>
                ...
                <div x-show="section === 'staff'" x-cloak x-transition.opacity>
                    <div class="bg-white shadow-2xl overflow-hidden border border-light overflow-x-auto">
                        <div class="p-10 border-b border-light flex justify-between items-center bg-white sticky top-0 z-10">
                            ...
                            <button @click="openAddModal('staff', '{{ route('admin.staff.store') }}')" class="btn-primary">+ Tambah Data</button>
                        </div>
                        <table class="w-full text-left whitespace-nowrap">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b border-light hover:bg-light/50 transition-colors">
                                    <td class="px-6 py-4 font-bold text-navy text-xs">{{ $person->name }}</td>
                                    <td class="px-6 py-4 text-xs text-slate-600">{{ $person->nuptk ?? '-' }}</td>
                                    <td class="px-6 py-4 text-xs text-slate-600">{{ $person->gender }}</td>
                                    <td class="px-6 py-4 text-xs text-slate-600">{{ $person->birth_place }}</td>
                                    <td class="px-6 py-4 text-xs text-slate-600">{{ \Carbon\Carbon::parse($person->birth_date)->format('d-m-Y') }}</td>
                                    <td class="px-6 py-4 text-xs text-slate-600">{{ $person->nip ?? '-' }}</td>
                                    <td class="px-6 py-4 text-xs font-bold text-primary uppercase">{{ $person->ptk_type }}</td>
                                    <td class="px-6 py-4">
                                        @if($person->image)
                                        <img src="{{ asset('storage/' . $person->image) }}" class="w-10 h-10 object-cover rounded-full border border-slate-200" alt="{{ $person->name }}">
                                        @else
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex gap-2">
                                            <button @click="openEditModal('staff', '{{ route('admin.staff.update', $person->id) }}', {{ $person }})" class="w-8 h-8 bg-blue-500 text-white flex items-center justify-center hover:bg-navy transition-all rounded"><i class="fa fa-edit text-xs"></i></button>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4" @click.self="showModal = false">
        <div class="bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" @click.stop>
            ...
            <form x-show="modalType === 'changePassword'" method="POST" action="{{ route('admin.password.change') }}" class="p-8 space-y-6">
                @csrf
                <div>
