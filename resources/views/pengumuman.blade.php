...
@section('title', 'Pengumuman - SMAN 11 KUPANG')
        ...
        <div class="container mx-auto px-4 lg:px-10">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-20">
                ...
                <div class="lg:col-span-4">
                    <div class="sticky top-32">
                        ...
                        <div class="bg-navy p-10 text-white">
                            ...
                            <a href="{{ route('kontak') }}" class="text-xs font-black uppercase tracking-widest text-primary hover:text-white transition-colors">Hubungi Kami <i class="fa fa-arrow-right ml-2"></i></a>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-8 space-y-12">
                    ...
                    <div class="group relative bg-white border border-light p-12 hover:shadow-2xl transition-all duration-500 {{ $item->priority === 'Urgent' ? 'border-l-8 border-l-secondary' : 'border-l-8 border-l-primary' }}">
                        <div class="flex items-center space-x-4 mb-6">
                            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-navy/40">{{ $item->created_at->format('d M Y') }}</span>
                            @if($item->priority === 'Urgent')
                                <span class="bg-secondary text-white text-[9px] font-black px-3 py-1 uppercase tracking-widest animate-pulse">Urgent</span>
                            @endif
                        </div>
                        <h3 class="text-2xl lg:text-3xl font-black text-navy uppercase tracking-tighter mb-6 group-hover:text-primary transition-colors leading-tight">{{ $item->title }}</h3>
                        <p class="text-text leading-relaxed font-medium mb-8">{{ $item->content }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
